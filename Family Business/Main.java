...
public class Main
{
	...
	public static final int NUM_OF_MOBSTERS = 6;
	...
	public static void playerTurn ( )
	{
		...
		if ( players[playerNumber] != null )
		{
			...
			do 
			{
				System.out.println( "Player " + players[playerNumber].getName ( ) +  ", which card do you want to play? (Enter number): ");
	...
	public static void killMobster ()
	{
		for ( int i = 0; i < players.length; i++ )
		{
			if ( players[i] != null )
			{
				...
				{
					if ( hitList.get(0).equals(players[i].getName())  )
					{
						deadMobsters[i]++;
	...
	public static boolean gameOverCheck ()
	{
		...
		if ( numDeadPlayers == ( numOfPlayers - 1) )
		{
			...
			{
				if ( deadMobsters[j] != NUM_OF_MOBSTERS )
				{
					winningIndex = j;
					j = numOfPlayers;
				}
			}
			System.out.println( "Game over, Player " + players[winningIndex].getName ( ) + " wins!"  );
			return true;
		}
		else
		{
